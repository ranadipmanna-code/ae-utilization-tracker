...
def main():
    ...
    with eng.begin() as conn:
        ...
        conn.execute(text("INSERT INTO user_roles (email, name, role) VALUES ('admin1@anudip.org','Admin One','admin')"))
        for email, name in CORE_AE.items():
            conn.execute(text("INSERT INTO user_roles (email, name, role) VALUES (:e,:n,'core_ae')"),
                         {"e": email, "n": name})
        for email, name in EXTENDED_AE.items():
            conn.execute(text("INSERT INTO user_roles (email, name, role) VALUES (:e,:n,'extended_ae')"),
                         {"e": email, "n": name})
